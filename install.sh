...
# BANTENG MERAH ## BUMEGAWATI WOWKOWOWKWK
# ===================
clear
...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To B-Liv Project Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}B-Liv Vpn ${NC}${YELLOW}(${NC} ${green}B-Liv Project ${NC}${YELLOW})${NC}"
echo -e "       © B-Liv Project ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
# USERNAME
rm -f /usr/bin/user
read -p "   โปรดใส่ชื่อที่สมัครสมาชิก : " userinput
username="$userinput"
echo "$username" >/usr/bin/user
expx="9999-1-1"
echo "9999-1-1" >/usr/bin/e
# DETAIL ORDER
...
# Status ExpiRED Active | MyridVPN
...
clear
# REPO    
    REPO="raw.githubusercontent.com/gotza02/mvip/main/"
...
function pasang_domain() {
    clear
    echo -e "   .----------------------------------."
    echo -e "   |กรุณาเลือกประเภทโดเมนด้านล่างนี้|"
    echo -e "   '----------------------------------'"
    echo -e "     1) โดเมนของตัวเอง"
    echo -e "     2) ใช้โดเมนสุ่ม พิเศษสำหรับ Digital Ocean หรือ ISP อื่น ✖️"
    echo -e "   ------------------------------------"
    read -p "   กรุณาเลือกหมายเลข 1 หรือ ปุ่มอื่นๆ (สุ่ม) : " host
    echo ""

    if [[ $host == "1" ]]; then
        echo -e "   กรุณาใส่ชื่อโดเมนย่อยของคุณ"
        read -p "   โดเมนย่อย: " host1
        # ตรวจสอบให้แน่ใจว่าได้ตั้งค่าอีเมลสำหรับการแจ้งเตือนการสำรองข้อมูล
        if [[ -z "$email" ]]; then
            echo -e " ใส่ชื่อของคุณเพื่อรับการแจ้งเตือนการสำรองข้อมูล"
            read -rp " ชื่อ: " email
            echo "$email" > /root/.email
        fi
        # สมมติว่าตั้งค่า IP ล่วงหน้าเป็นสิ่งจำเป็น
        echo "IP=" >> /var/lib/kyt/ipvps.conf
        echo "$host1" > /etc/xray/domain
        echo "$host1" > /root/domain
    elif [[ $host == "2" ]]; then
        # ติดตั้ง cf สำหรับการตั้งค่าโดเมนสุ่ม
        wget -O cf.sh ${REPO}//cf.sh && chmod +x cf.sh && ./cf.sh
        rm -f /root/cf.sh
    else
        echo "ใช้โดเมนย่อย/โดเมนสุ่ม"
        # ต้องการยังต้องการการตั้งค่าสำหรับการใช้โดเมนสุ่ม, ถ้ามี
    fi
    clear
}
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    curl https://raw.githubusercontent.com/gotza02/mvip/main/nginx.conf > /etc/nginx/nginx.conf
...
wget raw.githubusercontent.com/gotza02/mvip/main//limit.sh && chmod +x limit.sh && ./limit.sh
...
cd
clear
#SERVICE LIMIT ALL IP #####MASIH MAU DI FIXXS KU TANDAIN KAUU
...
print_install "Memasang modul SlowDNS Server"
    wget -q -O /tmp/nameserver "${REPO}/nameserver" >/dev/null 2>&1
...
function menu(){
    ...
    wget https://raw.githubusercontent.com/gotza02/mvip/main/menu.zip
    unzip menu.zip
    chmod +x menu/*
...
# LYNZ SUKA HAHAHAHAHAHAHHAHA AUTO SCRIPT BETA RILIS BULAN 12 2023 WOWKWOKWOWKWKWKOWK MANG EYA 
reboot